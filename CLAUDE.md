...
- **48h auto-approval**: `backend/src/lib/autoApprove.ts` exports `runAutoApprovalSweep(prisma)`, which finds `PENDING` returns with `createdAt` older than 48h and, per record, `$transaction`s an update to `AUTO_APPROVED` (`decidedBy: "SYSTEM"`) alongside an `AuditLog` (`action: "AUTO_APPROVED"`, `actor: "SYSTEM"`, `reason: "48h timeout reached"`). It's triggered two ways: `GET /api/cron/auto-approve` (`backend/src/app/api/cron/auto-approve/route.ts`), guarded by a `CRON_SECRET` bearer token (matches Vercel Cron's convention of sending `Authorization: Bearer $CRON_SECRET`); and `npm run auto-approve` (`backend/scripts/autoApproveExpiredReturns.ts`), a standalone tsx script (own `PrismaPg`/`PrismaClient`, same pattern as `prisma/seed.ts`) for local runs or an external scheduler. Neither is wired into an actual scheduler yet — something needs to hit the route (or run the script) periodically.
